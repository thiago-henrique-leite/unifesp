...
void send_file(int sockfd, req_t r) {
   ...
   {
        perror("open");
        send(sockfd, COD_ERROR_FILE, strlen(COD_ERROR_FILE), 0);
        return;
   }
...
void proc_request(int sockfd, req_t r){
    if(strcmp(r.method, REQ_GET) == 0) {
        ...
        send(sockfd, COD_ERROR_METHOD, strlen(COD_ERROR_METHOD), 0);
    }
}
...
int main(int argc, char **argv) {
    ...
    while(1) 
    {
        ...
        printf("\nConectado com cliente %s: %d - Posição na Fila: %d\n",
        ...
        printf("Aguardando requisição do cliente %s\n", inet_ntoa(caddr.sin_addr));

        /* 
            O código ficará parado na próxima linha aguardando o cliente enviar o request.
            Sendo assim, nunca voltamos ao início do While até que o request seja enviado. 
        */
